        ...
        if ($handle !== FALSE) {
            ...
            while (($data = fgetcsv($handle, 1000, ",")) !== FALSE) {
                ...
                try {
                    ...
                    $stmt_user_exists = $pdo->prepare("SELECT id FROM users WHERE email = ? OR employee_code = ?");
                    $stmt_user_exists->execute([$email, $employee_code]);
                    if ($stmt_user_exists->fetch()) {
                        $results[] = ['status' => 'error', 'message' => "Row $row_number: User email '$email' or Employee Code '$employee_code' already exists.", 'data' => implode(', ', $data)];
                        continue;
                    }
                    
                    $hashed_password = password_hash($password, PASSWORD_DEFAULT);
                    // Add employee_code to the insert statement
                    $sql = "INSERT INTO users (employee_code, full_name, email, password, role, direct_manager_id, must_change_password) VALUES (?, ?, ?, ?, ?, ?, 1)";
                    $pdo->prepare($sql)->execute([$employee_code, $full_name, $email, $hashed_password, $role, $direct_manager_id]);
                    $results[] = ['status' => 'success', 'message' => "Successfully created user '$full_name' with code '$employee_code'.", 'data' => implode(', ', $data)];
