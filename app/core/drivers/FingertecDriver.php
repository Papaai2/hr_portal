...
class FingertecDriver extends EnhancedBaseDriver {
    ...
    protected function sendCommand($command, $data = null, $expectResponse = true) {
        ...
        if ($this->isFakeDevice($this->host)) {
            return $this->handleFakeCommand($command, $data);
        }
    ...
    /**
     * Handle fake device commands for testing
     */
    protected function handleFakeCommand($command, $data = null) {
        $this->logInfo("Handling fake command: {$command}");
        
        switch ($command) {
            case $this->commands['GET_VERSION']:
                return 'FingerTec TA200 v2.1.4';
                
            case $this->commands['GET_SERIAL']:
                return 'FTC' . date('Ymd') . '001';
                
            case $this->commands['GET_USER_COUNT']:
                return '3';
                
            case $this->commands['GET_ATTENDANCE_COUNT']:
                return '20';
                
            case $this->commands['GET_USERS']:
                return $this->getFakeUsers();
                
            case $this->commands['GET_ATTENDANCE']:
                return $this->getFakeAttendanceLogs();
                
            case $this->commands['CLEAR_ATTENDANCE']:
                return true;
                
            case $this->commands['ENABLE_REALTIME']:
            case $this->commands['DISABLE_REALTIME']:
                return true;
                
            case $this->commands['SET_TIME']:
                return true;
    ...
    public function getDeviceName(): string {
        ...
        return $deviceInfo['model'] ?? 'FingerTec Device';
    }
